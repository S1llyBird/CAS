...
class SGLangRollout(BaseRollout):
    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        # Update with any additional kwargs
        request_sampling_params.update(kwargs)
        is_search_r1 = str(self.config.multi_turn.format).lower() == "search_r1"
        ...
        while current_turns < self.config.multi_turn.max_assistant_turns:
            if _req.state == AsyncRolloutRequestStateEnum.PENDING:
                ...
                if is_search_r1:
                    truncated_content = truncate_after_first_search_request(content)
                    if truncated_content != content:
                        content = truncated_content
                        # Ensure consistency after text-level truncation.
                        content_ids = None
                ...
                if finish_reason_type == FinishReasonTypeEnum.LENGTH:
                    ...
                    # ── Search-R1: <search>query</search> bypass ──────────────
                    search_query = extract_first_search_query(content) if is_search_r1 else None
                    if search_query and "search" in self._tool_map:
                        ...
                        if len(_req.input_ids) >= self.config.max_model_len:
                            ...
                            break
                        _req.state = AsyncRolloutRequestStateEnum.RUNNING
                    # ── end Search-R1 bypass ──────────────────────────────────
                    elif (not is_search_r1) and self._function_call_parser and self._function_call_parser.has_tool_call(content):
                        finish_reason_type = FinishReasonTypeEnum.TOOL_CALL
                        ...
                        elif is_search_r1 and not _ANSWER_TAG_RE.search(content):
                            # Search-R1 should continue until a complete <answer>...</answer> appears.
                            _req.state = AsyncRolloutRequestStateEnum.RUNNING
                        else:
    ...
    def _preprocess_prompt_to_async_rollout_requests(self, prompts: DataProto, n: int = 1) -> list[AsyncRolloutRequest]:
        ...
        for data_idx, (raw_prompt, multi_modal_data) in enumerate(
            ...
            if self._tool_schemas and not is_search_r1:
                ...
                _tools_kwargs = prompts.non_tensor_batch["tools_kwargs"][data_idx] if self._tool_schemas else {}
                _tool_schemas = None
            ...
            req = AsyncRolloutRequest(
                ...
                apply_chat_template_kwargs=self.config.multi_turn.apply_chat_template_kwargs,
