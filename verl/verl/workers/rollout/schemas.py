...
class AsyncRolloutRequest(BaseModel):
    ...
    def initialize_request(cls, values):
        ...
        apply_chat_template_kwargs = values.get("apply_chat_template_kwargs", {})
    ...
    def _handle_apply_chat_template(
        ...
        supported_kwargs = {}
        try:
            signature = inspect.signature(processing_class.apply_chat_template)
            supported_kwargs = {
                key: value for key, value in apply_chat_template_kwargs.items() if key in signature.parameters
            }
        except (TypeError, ValueError):
            supported_kwargs = apply_chat_template_kwargs
    ...
    def add_raw_information(
        self,
        processing_class: PreTrainedTokenizer | PreTrainedTokenizerFast | ProcessorMixin,
        text: str,
    ) -> None:
        """Inject <information>text</information> directly into input_ids without going through
        the chat template's tool role mechanism.  loss_mask is False (observation tokens).

        Used by the Search-R1 <search>/<information> tag bypass in sglang_rollout.py.
        """
        raw = f"<information>{text}</information>\n"
        if isinstance(processing_class, PreTrainedTokenizer) or isinstance(
            processing_class, PreTrainedTokenizerFast
        ):
            ids = processing_class(raw, return_tensors="pt", add_special_tokens=False)["input_ids"]
        elif isinstance(processing_class, ProcessorMixin):
            ids = processing_class(text=[raw], return_tensors="pt", add_special_tokens=False)["input_ids"]
        else:
            raise ValueError(f"Unsupported processing class type: {type(processing_class)}")
        self._remove_generation_prompt_ids_if_present()
        self._update_input_ids(processing_class, ids, attention_mask=True, loss_mask=False)
