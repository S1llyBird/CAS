...
class SearchR1ToolParser(ToolParser):
    """Search-R1 parser: parse <search>query</search> into search tool calls."""
    ...
    async def extract_tool_calls(self, responses_ids: list[int]) -> tuple[str, list[FunctionCall]]:
        loop = asyncio.get_running_loop()
        text = await loop.run_in_executor(None, self.tokenizer.decode, responses_ids)
        truncated_text = truncate_after_first_search_request(text)
        query = extract_first_search_query(truncated_text)
        if not query:
        ...
        return truncated_text, [function_call]
