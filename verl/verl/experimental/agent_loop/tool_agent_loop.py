...
from verl.tools.utils.search_r1_postprocess import truncate_after_first_search_request
...
_SEARCH_R1_ANSWER_RE = re.compile(r"<answer>.*?</answer>", re.DOTALL)
...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def _handle_generating_state(
        ...
        if self.tool_parser_name == "search_r1":
            assistant_message_raw = await self.loop.run_in_executor(
                None, lambda: self.tokenizer.decode(response_ids, skip_special_tokens=False)
            )
            truncated_message = truncate_after_first_search_request(assistant_message_raw)
            if truncated_message != assistant_message_raw:
                response_ids = await self.loop.run_in_executor(
                    None, lambda: self.tokenizer.encode(truncated_message, add_special_tokens=False)
                )
        ...
        has_final_answer = (
            self.tool_parser_name == "search_r1" and bool(_SEARCH_R1_ANSWER_RE.search(assistant_message))
        )
        ...
        elif self.tool_parser_name == "search_r1" and not has_final_answer:
            # Search-R1 should only terminate after <answer>...</answer> or max turn/length.
            return AgentState.GENERATING
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        # Search-R1 模式：不走 <tool_response> 机制，直接注入 <information>...</information>
        if self.tool_parser_name == "search_r1":
            ...
            for tool_response, tool_reward, _ in responses:
                ...
                parsed_text = tool_text
                try:
                    parsed_obj = json.loads(tool_text)
                    if isinstance(parsed_obj, dict) and "result" in parsed_obj:
                        parsed_text = str(parsed_obj.get("result", ""))
                except Exception:
                    # 非 JSON 结果按原样使用
                    pass

                if parsed_text:
                    information_blocks.append(f"<information>{parsed_text}</information>\n")

            if not information_blocks:
                information_blocks.append("<information>No search results found.</information>\n")

            raw_information = "".join(information_blocks)
            if self.processor is not None:
                model_inputs = self.processor(
                    text=[raw_information],
                    images=None,
                    return_tensors="pt",
                ...
                response_ids = await self.loop.run_in_executor(
                    None, lambda: self.tokenizer.encode(raw_information, add_special_tokens=False)
                )
    ...
    async def _call_tool(
        ...
        # Search-R1 模式下尽量保留完整检索信息，避免在这里提前截断。
        if self.tool_parser_name != "search_r1" and tool_response_text and len(tool_response_text) > self.max_tool_response_length:
