...
class AlgoConfig(BaseConfig):
    ...
    Args:
        ...
        dcr_enable (bool): Whether to enable DCR-GRPO adaptive answer confidence weighting.
        dcr_initial_scores_path (Optional[str]): Path to `initial_scores.pt` used to bootstrap ACI.
        aci_buffer_size (int): Max length of ACI score buffer.
        aci_alpha_init (float): Initial alpha_t for ACI.
        aci_alpha_target (float): Target alpha for adaptive alpha_t update.
        aci_gamma (float): Step size for alpha_t update.
        aci_ema_ratio (float): EMA update ratio for current batch error.
        aci_penalty_weight (float): Low-confidence policy-loss weight (< 1.0).
        aci_ema_err_init (float): Initial EMA error for ACI.
    """
    ...
    dcr_enable: bool = False
    dcr_initial_scores_path: Optional[str] = None
    # ACI controller hyperparameters
    aci_buffer_size: int = 2000
    aci_alpha_init: float = 0.25
    aci_alpha_target: float = 0.25
    aci_gamma: float = 0.005
    aci_ema_ratio: float = 0.05
    aci_penalty_weight: float = 0.1
    aci_ema_err_init: float = 0.0
    # Deprecated aliases (kept for backward compatibility)
    dcr_buffer_size: int = 2000
    dcr_alpha_init: float = 0.25
    dcr_ema_err_init: float = 0.0
