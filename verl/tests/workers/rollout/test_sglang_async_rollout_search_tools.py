...
DEFAULT_SEARCH_R1_USER_CONTENT_PREFIX = (
    "Answer the given question. You must conduct reasoning inside <think> and </think> "
    "first every time you get new information. After reasoning, if you find you lack "
    ...
    "You can search as many times as your want. Do not output <tool_call> or JSON "
    "function-call text. Only use the tag sequence above. If you find no further "
    "external knowledge needed, you can directly provide the answer inside <answer> "
    "and </answer>, without detailed illustrations. For example, <answer> Beijing "
    "</answer>. Question: "
)
search_r1_user_content = DEFAULT_SEARCH_R1_USER_CONTENT_PREFIX.rstrip("\n") + "How's the weather lately?"
...
class TestRolloutWithSearchTools:
    ...
    @pytest.fixture
    def search_r1_rollout_config(self, search_rollout_config):
        rollout_config = deepcopy(search_rollout_config)
        rollout_config.multi_turn.format = "search_r1"
        return rollout_config
    ...
    @pytest.fixture
    def search_r1_data_proto(self, qwen_tokenizer):
        messages = [[{"role": "user", "content": search_r1_user_content}]]
        prompts = [qwen_tokenizer.apply_chat_template(message, tokenize=False, add_generation_prompt=True) for message in messages]
        input_ids, attention_mask, position_ids = prepare_inputs(qwen_tokenizer, prompts, 1000)
        prompt_dict = TensorDict(
            {
                "input_ids": input_ids,
        ...
        return DataProto(
            batch=prompt_dict,
            non_tensor_batch={"raw_prompt": np.asarray(messages), "tools_kwargs": tools_kwargs, "index": index},
        )
    ...
    @pytest.fixture
    def mock_search_r1_rollout(self, search_r1_rollout_config, qwen_tokenizer, qwen_model_config):
        with (
            patch.object(SGLangRollout, "_init_distributed_env", return_value=None),
            patch.object(SGLangRollout, "_init_inference_engine", return_value=None),
            patch.object(SGLangRollout, "_init_sampling_params", return_value=None),
        ):
            rollout_config: RolloutConfig = omega_conf_to_dataclass(
                search_r1_rollout_config, dataclass_type=RolloutConfig
            )
            model_config = HFModelConfig(path=self.local_model_path)
            rollout = SGLangRollout(
                config=rollout_config,
            ...
            rollout.sampling_params = {
                "n": 1,
                "max_new_tokens": search_r1_rollout_config.response_length,
                "presence_penalty": 0.0,
                "frequency_penalty": 0.0,
                "repetition_penalty": 1.0,
            }
    ...
    def test_search_r1_req_creation_skips_tool_schema_injection(
        self,
        mock_search_r1_rollout,
        search_r1_data_proto,
    ):
        req_list = mock_search_r1_rollout._preprocess_prompt_to_async_rollout_requests(search_r1_data_proto, n=1)
        assert len(req_list) == 1
        assert req_list[0].state == AsyncRolloutRequestStateEnum.PENDING
        assert req_list[0].tool_schemas is None
        assert req_list[0].tools_kwargs == search_r1_data_proto.non_tensor_batch["tools_kwargs"][0]
        assert req_list[0].input_ids is not None
