...
CAS_DIR="$(dirname "$SCRIPT_DIR")"
VERL_METRICS_FILE="$CAS_DIR/verl/verl/trainer/ppo/metric_utils.py"
CAS_METRICS_FILE="$CAS_DIR/cas/utils/metric_utils.py"

echo "======================================================================"
echo "Setting up custom metrics for CAS GRPO training"
echo "======================================================================"
...
# Check if the CAS custom metrics file exists
if [ ! -f "$CAS_METRICS_FILE" ]; then
    echo "Error: CAS custom metrics file not found at $CAS_METRICS_FILE"
    exit 1
fi
...
# Copy CAS's custom metrics file to override VERL's version
echo "Installing CAS custom metrics..."
cp "$CAS_METRICS_FILE" "$VERL_METRICS_FILE"
