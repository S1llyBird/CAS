...
CALIB_DIR=${CALIB_DIR:-"$DATA_DIR/calibration_qwen3-8b"}
...
TOOL_CONFIG="$CONFIG_PATH/tools/search_tool_config_qwen3-8b.yaml"
REWARD_FUNCTION_PATH="$PROJECT_DIR/cas/reward_score"

BASE_MODEL="Qwen/Qwen3-8B"
PROJECT_NAME="CAS"
EXPERIMENT_NAME="CAS_qwen3-8b-instruct_grpo"
...
if [[ ! -f "$TRAIN_FILE" || ! -f "$INITIAL_SCORES_PATH" || ! -f "$LAMBDA_FIXED_PATH" ]]; then
    ...
    echo "Run recipe/cas/prepare_cas_aci_full_qwen3-8b.sh first." >&2
    exit 1
fi
...
python3 -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.rollout.multi_turn.apply_chat_template_kwargs.enable_thinking=False \
